...
class FeatureExtraction:
    ...
    def InfoEmail(self):
        try:
            if re.findall(r"mail\(|mailto:", self.response.text, re.IGNORECASE):
                return -1
            else:
                return 1
        except:
    ...
    def WebsiteTraffic(self):
        try:
            rank = BeautifulSoup(urllib.request.urlopen("http://data.alexa.com/data?cli=10&dat=s&url=" + self.url, timeout=5).read(), "xml").find("REACH")['RANK']
            if (int(rank) < 100000):
                return 1
            return 0
        except :
